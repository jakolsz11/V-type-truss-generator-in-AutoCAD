...
def draw_lines_m16(model_space, sheet):
    
    for row in range(281, 314):
        row_data = []
        ...
        print("Linia sruby m16 PD narysowana") 
        
          
def draw_lines_m20(model_space, sheet):
    
    for row in range(314, 378):
        row_data = []
        for col in range(4, 10):
            one_data = sheet.cell(row, col).value      
        ...
        print("Linia sruby m20 WP narysowana")   
...
def draw_sruby(doc):
    ...
    draw_lines_m16(model_space, sheet)
    draw_lines_m20(model_space, sheet)
